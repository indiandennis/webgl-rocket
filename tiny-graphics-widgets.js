...
const Canvas_Widget = widgets.Canvas_Widget =
    class Canvas_Widget {                           // **Canvas_Widget** embeds a WebGL demo onto a website in place of the given placeholder document
...
const Controls_Widget = widgets.Controls_Widget =
    class Controls_Widget {                                               // **Controls_Widget** adds an array of panels to the document, one per loaded
        ...
        render(time = 0) {                       // Check to see if we need to re-create the panels due to any scene being new.
            // Traverse all scenes and their children, recursively:
            const open_list = [...this.scenes];
            while (open_list.length) {
                open_list.push(...open_list[0].children);
...
const Code_Manager = widgets.Code_Manager =
    class Code_Manager {                                  // **Code_Manager** breaks up a string containing code (any ES6 JavaScript).  The RegEx being used
...
const Code_Widget = widgets.Code_Widget =
    class Code_Widget {                                         // **Code_Widget** draws a code navigator panel with inline links to the entire program source code.
        ...
        build_reader(element, main_scene, additional_scenes, definitions) {                                           // (Internal helper function)
...
//       text.textContent  = "Code for the above scene:";
        ...
        display_code(class_to_display) {                                           // display_code():  Populate the code textbox.
...
const Text_Widget = widgets.Text_Widget =
    class Text_Widget {                                                // **Text_Widget** generates HTML documentation and fills a panel with it.  This
